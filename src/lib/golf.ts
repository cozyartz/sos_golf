...
export type Course = { id: string; name: string; region: string; address: string; holes: Hole[]; teeSets: TeeSet[]; tapPoints: number; location?: { latitude: number; longitude: number; stateCode: string } };
...
export type RoundStatus = 'draft' | 'in_progress' | 'submitted' | 'verified' | 'rejected';
